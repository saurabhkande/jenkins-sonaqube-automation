...
EOF'
    sudo apt update -y
    sudo sudo apt-get -y install openjdk-17-jdk
...
# Postgres Database installation and setup
    ...
    sudo echo "postgres:ssk123" | sudo chpasswd
    sudo runuser -l postgres -c "createuser sonar"
    sudo -i -u postgres psql -c "ALTER USER sonar WITH ENCRYPTED PASSWORD 'ssk123';"
